    ...
    def fit(self, X, y=None, feature_names=None):
        ...
        if 'r' in self.model_type:
            ...
            if not self.exp_rand_tree_size: # simply fit with constant tree size
                ...
                if type(self.tree_generator) in [GradientBoostingRegressor,RandomForestRegressor]:
                    self.tree_generator.set_params(warm_start=True) 
                    curr_est_=0
                    for i_size in np.arange(len(tree_sizes)):
                        #print(str(curr_est_)+'/' + str(len(tree_sizes))) 
            ...
            if type(self.tree_generator) in [GradientBoostingRegressor,RandomForestRegressor]:
                ...
                tree_list = self.tree_generator.get_booster().get_dump()
                if isinstance(self.tree_generator, XGBRegressor) or isinstance(self.tree_generator, XGBClassifier):
                     tree_list = [[x] for x in self.tree_generator.get_booster().get_dump()]
            ## extract rules
